...
cour=open("Merged Data Sheet.txt",'r')
...
for i in courfh:
    ...
    if i[0].startswith("Participant ID"):
        ...
        search = str(i[part]+"_"+i[dis]+"_"+i[ass])# removing Category (+"_"+i[cat]) from the search list to check the count
        if search in dict:
            continue
        else:
            dict[search]=0
...
for value in ph:
    ...
    for x,y in dict.items():
        x=x.rstrip().split("_")
        if x[1] == diseasename: # do not put "in" it will take non- small cell and small cell
            if x[2]=="Archival":
